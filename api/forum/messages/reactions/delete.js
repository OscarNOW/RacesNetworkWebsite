//forum/messages/reactions/delete?message=<>&isDiscord=<>&logInfo1=<>&logInfo2=<>
module.exports = {
    async execute(error, end, req, extra, params) {
        ...
        if (!((verified.user == extra.messages.list[messageId].author) || (extra.roles[extra.users.list[verified.user].role].permissions.includes('deleteOtherReaction')))) return error(401)

        let reactions = extra.messages.list[extra.messages.list[messageId].parent].reactions
        reactions.splice(reactions.indexOf(messageId), 1)
