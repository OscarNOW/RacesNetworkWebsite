...
function onLoad() {
    ...
    //*
    var slides = document.getElementsByClassName("onlyJS");
    var i = 0;
    while (i < slides.length) {
        slides.item(i).style.display = '';
        //slides.item(i).classList.remove("onlyJS");
        i++
    } //*/
...
function staffUpdate() {
    document.getElementById('staffWrapper').innerHTML = '';
    httpGetAsync(`http://${window.location.href.split('/')[2]}//users/staff`, (ret) => {
...
function createStaffTile(val) {

    /*
    <a href="Staff/derpendkatje/" style="color:white; text-decoration: none;">
        <div class="StaffCard">
            <div></div>
            <p></p>
        </div>
    ...
    newA.href = `${val.staffPage}&redirect=http://${window.location.href.split('/')[2]}/Staff`
    newA.style = `color: white; text-decoration: none;`
    ...
    document.getElementById('staffWrapper').appendChild(newA)
