...
//#endregion
//#endregion
let url = `http://${window.location.href.split('/')[2]}//users/special/forumWelcome`
...
async function onload() {
    ...
    let newUrl = 'http://' + window.location.href.split('/')[2] + `/${newUser.avatar}`

    document.getElementById('messageIcon').style.backgroundImage = `url(${newUrl})`
    document.getElementById('messageIcon').style.backgroundColor = 'white'
    document.getElementById('messageIconLink').href = `http://${window.location.href.split('/')[2]}/users/${newUser.id}`

    log.callback(() => {
        if (log.loggedIn) {
            document.getElementById('logOut').style.display = null;
            document.getElementById('accountButton').style.display = null;
        } else {
            document.getElementById('post').innerText = 'Login'
            document.getElementById('post').href = '#'
            document.getElementById('post').onclick = log.login
        }
        if (log.hasPermission('seeMessageOverview')) {
            let messageOverview = document.getElementById('messageOverview')
            messageOverview.style.display = null;

            document.getElementById('messageOverviewUnban').style.display = null;
            document.getElementById('messageOverviewUnmute').style.display = null;
            document.getElementById('messageOverviewUnwarn').style.display = null;
        }
    })
...
function accountPage() {
    if (!log.loggedIn) return;

    window.location.replace(`http://${window.location.href.split('/')[2]}/user?user=${log.userId}`)
}