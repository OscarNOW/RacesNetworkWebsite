...
function onload() {
    ...
    document.getElementById('dropdown-content').onmouseenter = () => updateDropdown(() => isInDropdown = true)
    document.getElementById('dropdown-content').onmouseleave = () => updateDropdown(() => isInDropdown = false)

    let isOnButton = false;
    document.getElementById('dropdown-button').onmouseenter = () => updateDropdown(() => isOnButton = true)
    document.getElementById('dropdown-button').onmouseleave = () => updateDropdown(() => isOnButton = false)

    function updateDropdown(code) {
        code()
        if (isInDropdown || isOnButton) {
            document.getElementById('dropdown-content').style.display = 'flex'
        } else {
            document.getElementById('dropdown-content').style.display = null
        }
    }

    function doneLoad() {
        try {
            window.parent.doneLoad();
        } catch {
            document.getElementById('body').parentElement.removeChild(document.getElementById('body'))
            window.location.replace(`http://${window.location.href.split('/')[2]}/forum/messages/load`)
        }
    }

    log.callback(() => {
        if (!log.loggedIn) return doneLoad()

        updateMessages(() => {

            let defaultFilter = document.getElementById('defaultFilter').className
            if (defaultFilter == '|filter|') defaultFilter = null;
            ...
            if (defaultFilter) {
                ...
                dropdownClick(pair[defaultFilter])
            }
...
function createMessage(author, tagg, date, text, id) {
    return new Promise(resolve => {
        let div = document.createElement('div')
        div.onclick = () => {
            window.parent.open(`http://${window.location.href.split('/')[2]}/forum/message?message=${id}`)
        }
        div.innerHTML = `
        <div class="messageUserIcon" style="background-image: url(/data/usericons/${author}.png)"></div>
        <div class="messageAttributeContainer">
...
function dropdownClick(num) {
    for (let ii = 1; ii < 5; ii++) {
        document.getElementById(`dropdown${ii}`).style.display = null
    }
    document.getElementById(`dropdown${num}`).style.display = 'none'

    let text = document.getElementById(`dropdownP${num}`).innerText
    document.getElementById('dropdownP').innerText = text

    filter = text.toLowerCase();
    if (filter == 'geen') filter = null;
...
function updateMessages(callback) {
    ...
    while (!(document.getElementById('messageContainer').childElementCount == 0)) {

        let children = document.getElementById('messageContainer').children
    ...
    let link = `http://${window.location.href.split('/')[2]}//forum/messages/fetch?isDiscord=${log.isDiscord}&logInfo1=${log.logInfo1}&logInfo2=${log.logInfo2}`
    if (filter) link += `&filter=${filter}`

    let doMessages = messages => {
        messages.forEach(async val => {
            await createMessage(val.author, val.tagg, val.date, val.text, val.id)
        })
        if (callback) callback()
    }

    if (fetchResponse[link]) {
        doMessages(fetchResponse[link]);
    } else {

        $.getJSON(link, messages => {
            fetchResponse[link] = messages
