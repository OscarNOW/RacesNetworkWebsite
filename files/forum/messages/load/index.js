...
function onload() {
    filter = document.getElementById('filter').className
    if (filter == '|filter|') filter = null;

    log.callback(() => {
        if (log.hasPermission('seeMessageOverview')) {
            let srcLink = `/forum/messages?isDiscord=${log.isDiscord}&logInfo1=${log.logInfo1}&logInfo2=${log.logInfo2}`
            if (filter) srcLink += `&filter=${filter}`

            document.getElementById('iframe').src = srcLink
        } else {
            document.getElementById('iframe').src = `/error/401`
        }
    })
}
