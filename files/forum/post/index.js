...
function dropdownClick(num) {
    document.getElementById('post').style.cursor = null
    if (num == 4) {
        document.getElementById('allowReactions').style.display = 'initial'
    } else {
        document.getElementById('allowReactions').style.display = 'none'
    }
    for (let ii = 1; ii < 5; ii++) {
        if ((ii == 4) && (!log.hasPermission('makeAnnouncement'))) {
            document.getElementById(`dropdown${ii}`).style.display = 'none'
        } else {
    ...
    let text = document.getElementById(`dropdown${num}`).innerText
...
function onload() {
    ...
    log.callback(() => {
        if (!log.hasPermission('makeAnnouncement')) {
    ...
    document.getElementById('urlText').addEventListener("keyup", function (event) {
        if (event.keyCode == 13) {
            ...
            document.getElementById('text').value = document.getElementById('text').value.replace(selectedText, text)
...
function updateCookie() {
    ...
    document.getElementById('count').innerText = `${document.getElementById('text').value.length}/${document.getElementById('text').maxLength}`
...
function showToolTip(selection, force) {
    if (selection) {
        ...
        if (!(div.mouseIsOver)) {
            document.getElementById('toolTip').style.top = `calc(${mY}px - 3vmax - 25px)`
...
function post() {
    ...
    document.cookie = `text=Text...; path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT`;

    let newUrl = `http://${window.location.href.split('/')[2]}//forum/messages/post`
    let data = {
    ...
    if (tagg.toLowerCase() == 'melding') {
        data.allowReactions = document.getElementById('allowReactionsCheckbox').checked
    }

    let messageId = random()
    $.post(newUrl, data, (data, status) => {

        let url = `http://${window.location.href.split('/')[2]}/forum/messages/${messageId}`
