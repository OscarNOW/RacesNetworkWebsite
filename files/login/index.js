...
function onload() {
    ...
    type = document.getElementById('type').className
    if (type == '|type|') type = null;

    if (type == 'staff') {
        document.getElementById('staffInfo').style.display = null;
    }

    log.callback(() => {
        //if (log.loggedIn) if (log.user) window.open(`http://${window.location.href.split('/')[2]}/users/${log.userId}/`, '_self')
    })
    ...
    if (redirect) {
        ...
        if (getCookie('redirect')) {
            document.cookie = `redirect=${redirect}; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/`;
        }
    }

    if (redirect) document.getElementById('register').href = `/register?redirect=${redirect}`
    ...
    document.getElementById('passwordShow').onclick = function () {
        if (document.getElementById('password').type == 'password') {
            ...
            document.getElementById('passwordShow').style.backgroundImage = 'url("../pics/Eye_line.png")'
        } else {
            document.getElementById('password').type = 'password'
            document.getElementById('passwordShow').style.backgroundImage = 'url("../pics/Eye.png")'
        }
    }

    if (redirect) {
        document.getElementById('border').style.border = 'none'
        document.getElementById('iframe').src = redirect
        document.getElementById('iframeShade').onclick = () => {
            window.open(redirect, "_self")
        }
    } else {
        document.getElementById('iframe').style.display = 'none'
        document.getElementById('iframeShade').style.display = 'none'
    }
...
function login() {
    ...
    let obj = checkLogin(email, password)

    if (obj.account) {
        //Succesvol ingelogd

        document.cookie = `user=${obj.account}; path=/; max-age=${60 * 60 * 24 * 7}`;
        document.cookie = `password=${password}; path=/; max-age=${60 * 60 * 24 * 7}`;

        let url = redirect
        if (!redirect) url = `http://${window.location.href.split('/')[2]}/users/${obj.account}`

        window.open(url, "_self")

    } else {
        if (obj.email) {
            //Email goed password niet
            document.getElementById('response').innerText = 'Wachtwoord is onjuist'
        } else if (obj.password) {
            //Password goed email niet
            document.getElementById('response').innerText = 'Email is onjuist'
        } else {
            //Beide niet goed
            document.getElementById('response').innerText = 'Wachtwoord en email zijn onjuist'
        }
    }
...
function checkLogin(email, password) {
    ...
    let userList = Object.keys(users.list)
    userList.forEach(val => {
        if (users.list[val].discord) return;
        if (email == users.list[val].email) if (owf(password) == users.list[val].password) correctAccount = val;
        if (!emailCorrect) {
            if (email == users.list[val].email) emailCorrect = true;
            if (owf(password) == users.list[val].password) passwordCorrect = true;
        }
    })
...
function removeStaffPopup() {
    document.getElementById('staffInfo').parentElement.removeChild(document.getElementById('staffInfo'));
}